...
export default function UserFormScreen({ route, navigation }: any) {
    ...
    // Carrega perfis, empresas e avatar
    useEffect(() => {
        (async () => {
            try {
                const resp = await api.get('/profile');
    ...
    const handleSubmit = async () => {
        if (!name.trim() || !email.trim() || profileId === 0) {
        ...
        formData.append('user_profile_id', String(profileId));
        if (companyId) formData.append('company_id', String(companyId));
        if (avatarUri && file) {
            const uri = avatarUri.startsWith('file://') ? avatarUri : `file://${avatarUri}`;
            formData.append('file', { uri, name: file.name, type: file.type } as any);
        }

        try {
            setLoading(true);
            const endpoint = editingUser ? `/user/${editingUser.user_id}` : '/user';
            const method = editingUser ? api.put : api.post;
    ...
    return (
        ...
        >
            ...
            {/* Só uma vez no App.tsx, não aqui! */}
            {/* <Toast /> */}
        </KeyboardAvoidingView>
    );
}
